...
- ✅ **9 témat opraveno** (`hints?: string[]` override v `gen()`, nebo přímá úprava `hints[]`): `nasobilka2345`, `rovnobezkyAKolmice`, `slovniDruhyUrcovaniVsechDesetiOhebneANeohebne`, `mapaStranySveta`, `slovesaMluvnickeKategorieCasovaniVJednoduchychCasech`, `vetaJednoduchaASouvetiVzorecSouveti`, `vypravovaniSCasovouPosloupnostiOsnova`, `stavbaVetyZakladniSkladebniDvojicePodmetPrisudek`, `zivaNezivaPrivroda`.
- 📊 **Postup:** `hint_leak` 804 → **352** (−56 %), témat 91 → **88**. Rozložení ploché (max 11/téma) — zbývá manuální práce téma po tématu dle žebříčku v `PROJECT_STATUS.md` §6.
