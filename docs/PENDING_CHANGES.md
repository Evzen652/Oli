...
- [ ] Dějepis g6 pokračování — **drag_order** (chronologie epoch pravěku) + **categorize** (historické prameny hmotné/písemné/obrazové) → ověří i tyto typy dle plánu.
- [ ] Z pilotu odvodit `TEMPLATE_STUPEN2.ts` (výpočetní + faktický) + odborná pravidla do `grade-6/README.md`.
- [ ] Navigation registr `grade-6/navigation.ts` + `displayNames.ts` (až bude víc témat napříč fyzikou i dějepisem).
- ⚠️ Grade 6 je „má obsah" → odemčen v onboardingu (6 fyzikálních + 1 dějepisné téma).
