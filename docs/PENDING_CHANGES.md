...
- ✅ **65 témat opraveno** (`hints?: string[]` override v `gen()`, nebo přímá úprava `hints[]`): viz plný seznam v `PROJECT_STATUS.md` §6 (nejnověji přidané: `minulostRegionuPovesti`, `slovaSouznacnaAProtikladna`, `sebekontrolaPisemnehoProjevu`, `popisPredmetuZvireteOsoby`, `vypravovaniOsnova`, `reprodukcePrectenehoTextu`, `popisPredmetuOsobyAPracovnihoPostupu`, `hlavniPostavyAJejichCharakteristika`, `reprodukcePrimereneSlozitehoSdeleni`).
...
- 🐞 **Druhá vlastní regrese** (`hlavniPostavyAJejichCharakteristika`) zachycena hned korpusovým ověřením před commitem — potvrzuje, že rutinní ověření po každé opravě funguje. Detail v `PROJECT_STATUS.md` §6.
- 📊 **Postup:** `hint_leak` 804 → **~56** (−93 %), témat 91 → **35** (měřeno opakovaně, robustnější než jednorázový běh). Zbývá manuální práce téma po tématu dle žebříčku v `PROJECT_STATUS.md` §6.
