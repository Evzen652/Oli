...
- **Ověřeno izolovaným renderem obou komponent s mock daty** (2 vs. 12 položek) — sandbox nemá síťový přístup k produkční Supabase (`403` na `uusaczibimqvaazpaopy.supabase.co` z organizační politiky), takže živé `/parent` ověření nebylo možné; layout ověřen přímým mountem se stejnými mock daty jaká `ParentDashboard` používá v demo režimu. Krátký seznam: výška podle obsahu (228/404 px), bez scrollbaru. Dlouhý seznam: ořízne se na 420 px, `scrollHeight` > `clientHeight` → scroll se aktivuje jen když obsah skutečně přeteče.
- **Ověřeno:** typecheck 0, `npx vitest run` 115/115 souborů a 4713/4713 testů, `vite build` prošel.
...
- ✅ **Vyřešeno (2026-08-25, pokračování 6):** dvě sekce s pevnou `h-[460px]` a vnořeným scrollem opraveny přímo v komponentách — viz níže.
