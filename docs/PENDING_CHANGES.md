...
- 🟡 **Zbývá (zapsáno v `DESIGN_SYSTEM.md`):** ~30 ručně psaných stavových pilulek (většina v adminu) na `Badge` varianty; `ProgressIndicator`/`MiniExplainer` píšou barvy natvrdo. Dark mode a admin zůstávají odložené dle původního rozhodnutí.
