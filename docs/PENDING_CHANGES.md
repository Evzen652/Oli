...
- [ ] Zbylá fyzika g6: **hustota** (ρ=m/V vrchol — staví na hmotnosti+objemu), teplota, čas.
- [ ] Dějepis g6 (faktický vzor — authoring pipeline).
- [ ] Z pilotu odvodit `TEMPLATE_STUPEN2.ts` + odborná pravidla do `grade-6/README.md`.
- [ ] Navigation registr `grade-6/navigation.ts` + `displayNames.ts` (až bude víc témat; teď fyzika plochá).
- ⚠️ Grade 6 je teď „má obsah" → odemčen v onboardingu (zatím 1 téma — pilot).
