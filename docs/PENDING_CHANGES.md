...
- ✅ **45 témat opraveno** (`hints?: string[]` override v `gen()`, nebo přímá úprava `hints[]`): viz plný seznam v `PROJECT_STATUS.md` §6 (nejnověji přidané: `slovesaZpusobOznamovaciRozkazovaciPodminovaci`, `etapyLidskehoZivotaDospivani`, `rysovaaniUseckyODaneDelce`, `crSymboly`, `velkaPismenaVlastniJmena`, `dopisPsaniSoukromehoDopisu`).
...
- 🔎 **Nový poznatek**: u kategorických otázek (etapy/kategorie), kde sdílená nápověda pojmenovává hledaný pojem přímo, funguje lépe katalogová výjimka (rejstřík VŠECH kandidátů s rozlišujícím údajem) než opis — viz `etapyLidskehoZivotaDospivani`.
- 📊 **Postup:** `hint_leak` 804 → **118** (−85 %), témat 91 → **53** (měřeno opakovaně, robustnější než jednorázový běh). Zbývá manuální práce téma po tématu dle žebříčku v `PROJECT_STATUS.md` §6.
