...
- [ ] **Ostrý běh workflow** na dalším batchi dějepisu g6 (zbývá 21 témat) — vyžaduje výslovný pokyn uživatele (placené agenty). Pilot pipeline.
- [ ] 🔎 **Zjemnit `hint_leak` check** v `contentAudit.ts` — flaguje i jednotku odpovědi („století" v „15. století"), kde se rozlišující číslo neprozrazuje = false positive. Brána ho proto jen reportuje (neblokuje). Ke zvážení: vyjmout jednotku/kategorii z kontroly. (Týká se i 3 hotových témat dějepisu — nejsou to reálné vady, jen šum checku.)
