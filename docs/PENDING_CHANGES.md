...
- ✅ **39 témat opraveno** (`hints?: string[]` override v `gen()`, nebo přímá úprava `hints[]`): viz plný seznam v `PROJECT_STATUS.md` §6 (nejnověji přidané: 3 algoritmické generátory matematiky, `casovaPrimkaGenerace`, `vetaJednoduchaSouveti`, a `zajmenaDruhyZajmen` — u posledního šlo o opravu chybné shody rodu, ne jen nápovědy).
...
- 📊 **Postup:** `hint_leak` 804 → **~120** (−85 %), témat 91 → **~56** (měřeno opakovaně, robustnější než jednorázový běh). Zbývá manuální práce téma po tématu dle žebříčku v `PROJECT_STATUS.md` §6.
