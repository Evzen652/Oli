...
- ✅ **Pushnuto do `main`** (potvrzeno Evženem, = nasazení) a předáno na druhý PC.
...
- ✅ **Vercel zase deployuje z pushů do `main`** (opraveno 10. 9.).
  Projekt měl `link.sourceless: true` — vazbu, která existuje jen jako
  metadata, takže `git connect` hlásil „already connected", ale z pushů se
  nestavělo; produkční větev přitom byla správně `main`, takže hledání
  chyby tam vedlo do slepé uličky. Pod tím leželo, že Vercel App neměla
  k repozitáři na GitHubu přístup. Poslední nasazení z Gitu předtím bylo
  **19. července** — sedm týdnů se nasazovalo jen z CLI, aniž by to někdo
  pojmenoval. Postup i způsob diagnostiky v `docs/PENDING_CHANGES.md`.
