...
export function SkillDetailModal({ childId, skillId, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // Individuální logy posledního sezení
      const lastSessionId = result[0]?.sessionId;
      if (lastSessionId) {
        const lastLogs = data.filter(l => l.session_id === lastSessionId);
        const items: LogItem[] = [...lastLogs].reverse().map(l => ({
          id: l.id as string,
          correct: l.correct as boolean,
          helpUsed: l.help_used as boolean,
          errorType: l.error_type as string | null,
          question: (l as any).question_text ?? undefined,
          correctAnswer: (l as any).correct_answer ?? undefined,
        }));
        setLogItems(items);
      }
