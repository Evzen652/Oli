...
export function ChildHomePage({ grade, onSelectTopic, onBrowseTopics }: ChildHomePageProps) {
  ...
  const [assignmentDateFilter, setAssignmentDateFilter] = useState<"all" | "today" | "week" | "older">("all");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const resolveAssignment = (a: typeof rawPending[0]): Assignment => {
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-black/[0.05] overflow-hidden">
          ...
          <div className="px-5 pt-3 pb-1">
            <div className="inline-flex rounded-xl border border-slate-200 bg-slate-50 p-0.5 gap-0.5">
              {(["all", "today", "week", "older"] as const).map(f => {
                const labels = { all: "Vše", today: "Dnes", week: "Tento týden", older: "Starší" };
                return (
