...
const POOL: { q: string; a: string; opts: string[]; e: string; hints?: string[] }[] = [
  { q: "Co musí obsahovat omluvenka?", a: "Koho omlouváme, proč, datum a podpis rodiče", opts: ["Koho omlouváme, proč, datum a podpis rodiče", "Jen datum", "Jen jméno žáka", "Básničku a přání"], e: "Omluvenka musí říct, KDO chyběl, PROČ chyběl, KDY chyběl a rodič ji musí podepsat — jinak ji učitel nemůže přijmout. Jen datum nebo jen jméno nestačí, protože učitel potřebuje všechny informace najednou.", hints: [
    "Přemýšlej, kolik různých věcí učitel potřebuje vědět, aby omluvenku uznal — stačí mu jedna z nich, nebo potřebuje víc najednou?",
  ...
  { q: "Co musí obsahovat pozvánka?", a: "Co, kdy, kde a kdo zve", opts: ["Co, kdy, kde a kdo zve", "Jen datum", "Jen adresu", "Jen jméno hostů"], e: "Dobrá pozvánka musí odpovědět na čtyři otázky: CO se koná (oslava, divadlo…), KDY to bude, KDE to bude a KDO zve. Bez těchto informací by pozvaný nevěděl, kam a kdy přijít.", hints: [
    "Přemýšlej, co všechno potřebuje vědět pozvaný, aby vůbec dorazil na správné místo ve správný čas — stačí mu jedna informace, nebo potřebuje víc najednou?",
  ...
  { q: "Vzkaz kamarádovi obsahuje:", a: "Kdo píše, co sděluje, případně kdy a kde", opts: ["Kdo píše, co sděluje, případně kdy a kde", "Jen básničku", "Omluvu vždy", "Velký příběh"], e: "Vzkaz kamarádovi by měl říct, KDO píše (aby kamarád věděl, od koho to je), CO chce sdělit a případně KDY a KDE se něco stane. Básnička nebo dlouhý příběh nejsou vzkazem — mají jiný účel.", hints: [
    "Přemýšlej, co potřebuje kamarád vědět, aby vzkazu rozuměl — je to jen jedna konkrétní věc (báseň, omluva, dlouhé vyprávění), nebo víc různých informací dohromady?",
...
function gen(level: number): PracticeTask[] {
  return shuffle(POOL).slice(0, 16).map(({ q, a, opts, e, hints }) => ({
    ...
    hints: hints ?? ["Omluvenka: kdo, proč, datum, podpis.", "Pozvánka: co, kdy, kde, kdo zve."],
    explanation: e,
  }));
}
