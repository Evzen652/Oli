...
const TEXTY = [
  {
    ...
    otazky: [
      ...
      { q: "Čím se živí velryby?", a: "Drobnými krevetami zvanými kril", opts: ["Mořskými řasami a trávou", "Drobnými krevetami zvanými kril", "Většími mořskými rybami", "Medúzami a chobotnicemi"], e: "Text přímo říká, že velryby se živí drobnými krevetami zvanými kril — to je překvapivé, protože jsou tak obrovské, ale jedí tak malé živočichy." },
...
const TEXT_L3 = {
  text: "Mravenci žijí ve velkých skupinách zvaných mraveniště. Jedno mraveniště může mít až milion mravenců. Mravenci si mezi sebou rozdělují práci — jedni hledají potravu, jiní hlídají mraveniště a královna klade vajíčka. Mravenec dokáže unést předmět, který je až 50krát těžší než on sám.",
  otazky: [
    ...
    { q: "Kolik mravenců může mít jedno mraveniště?", a: "Až milion", opts: ["Až milion", "Až tisíc", "Až sto", "Až deset tisíc"], e: "Text uvádí přímo: 'Jedno mraveniště může mít až milion mravenců.'" },
...
const TEORIE_L3: { q: string; a: string; opts: string[]; e: string }[] = [
  { q: "Jak nejrychleji najdeš konkrétní číslo v dlouhém textu?", a: "Přeletíš text očima", opts: ["Počítáš písmena ve slovech", "Přeletíš text očima", "Čteš text odzadu", "Hledáš nejdelší větu"], e: "Při hledání konkrétního čísla nemusíme číst každé slovo — stačí přelétnout text očima a všímat si čísel a slov kolem nich." },
  ...
  { q: "Proč je důležité ověřit, že nalezená informace opravdu odpovídá na otázku, ne jen že obsahuje stejné slovo?", a: "Stejné slovo může být jinde", opts: ["Stačí najít stejné slovo", "Stejné slovo může být jinde", "Text má vždy jen jednu informaci", "Slova se v textu neopakují"], e: "Slovo z otázky se může v textu objevit vícekrát a v různém významu, proto musíme ověřit, že věta se slovem opravdu odpovídá na to, na co se ptáme." },
  { q: "Jak se liší 'vyhledání konkrétní informace' od 'pochopení hlavní myšlenky' textu?", a: "Vyhledání je fakt, pochopení celek", opts: ["Vyhledání je vždy těžší", "Vyhledání je fakt, pochopení celek", "Hlavní myšlenka je v nadpisu", "Jsou to stejné dovednosti"], e: "Vyhledání je hledání konkrétního detailu (čísla, jména), kdežto pochopení hlavní myšlenky znamená umět celý text shrnout vlastními slovy." },
...
const TEORIE: { q: string; a: string; opts: string[]; e: string }[] = [
  { q: "Co jsou klíčová slova v textu?", a: "Nejdůležitější slova textu", opts: ["Slova, která neznáme", "Nejdůležitější slova textu", "Nejdelší slova v textu", "První slova každé věty"], e: "Klíčová slova jsou ta nejdůležitější — říkají, o čem celý text je. Nejsou to nutně nejdelší slova ani slova, která neznáme." },
