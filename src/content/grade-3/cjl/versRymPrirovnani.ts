...
const POOL_L1: QA[] = [
  { q: "Co je rým?", a: "Stejně nebo podobně znějící konec slov (pes – les)", opts: ["Rytmus celé básně, ne konce slov", "Stejně nebo podobně znějící konec slov (pes – les)", "Délka jednoho verše básně", "Opakování stejných slov v básni"], e: "Rým vzniká, když dvě slova mají stejné nebo podobné zakončení — třeba 'pes' a 'les' obě končí na '-es'. Rytmus, délka nebo opakování slov nejsou rým." },
  {
    q: "Co je verš?",
  ...
  },
  {
    ...
    opts: ["Rým, který je na konci verše", "Skupina veršů v básni (jako odstavec v próze)", "Celá báseň od začátku do konce", "Jeden jediný verš básně"],
  ...
  { q: "Co je přirovnání?", a: "Porovnání dvou věcí pomocí slova 'jako'", opts: ["Ponaučení, které si odneseme z bajky", "Porovnání dvou věcí pomocí slova 'jako'", "Název celé básně nebo díla", "Rým na konci dvou veršů"], e: "Přirovnání vždy používá slovo 'jako' — například 'silný jako lev'. Tím říkáme, že se jedna věc podobá druhé. Rým, název básně ani ponaučení přirovnání nejsou." },
  ...
  { q: "Věta: 'Máša je chytrá jako liška.' Co je to?", a: "Přirovnání (jako liška)", opts: ["Rým na konci věty", "Přirovnání (jako liška)", "Krátká bajka o lišce", "Pohádka o chytré Máše"], e: "Ve větě je slovo 'jako', které porovnává Mášinu chytrost s liškou. To je přirovnání. Rým by musel mít dvě slova se stejným zakončením." },
];
...
const POOL_L2: QA[] = [
  ...
  { q: "Přirovnání 'Bílý jako sníh' popisuje:", a: "Barvu (bílá) pomocí přirovnání ke sněhu", opts: ["Rychlost, jakou sníh padá", "Barvu (bílá) pomocí přirovnání ke sněhu", "Tvar jednotlivých vloček sněhu", "Chlad, který sníh vydává"], e: "Slovo 'bílý' říká, jakou má věc barvu, a přirovnání 'jako sníh' nám pomáhá si tu bílou barvu lépe představit. Nejde o chlad, tvar ani rychlost." },
...
const POOL_L3: QA[] = [
  ...
  { q: "Přečti básničku: 'Kočka leze po skále, / hraje si tam stále. / Pod oknem sedí pes, / dívá se, kam zmizel les.' Jaké je rýmové schéma (které verše se rýmují)?", a: "1. a 2. verš (skále–stále) a 3. a 4. verš (pes–les)", opts: ["1. a 4. verš se prý rýmují spolu", "1. a 2. verš (skále–stále) a 3. a 4. verš (pes–les)", "2. a 3. verš se prý rýmují spolu", "Žádné dva verše se v básni nerýmují"], e: "První dva verše končí na 'skále' a 'stále' (rým -ále), druhé dva na 'pes' a 'les' (rým -es). Vznikají tak dvě dvojice rýmujících se veršů." },
