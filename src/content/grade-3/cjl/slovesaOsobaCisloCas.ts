...
const L2: L2Item[] = [
  ...
  { i: 3, par: ["maloval jsem", "maloval jsi", "maloval", "malovali jsme", "malovali jste", "malovali"], tip: "V minulém čase prozradí osobu slůvko za slovesem („jsem, jsi, jsme, jste“)." },
  { i: 1, par: ["psala jsem", "psala jsi", "psala", "psaly jsme", "psaly jste", "psaly"], zajmena: ["já", "ty", "ona", "my", "vy", "ony"], tip: "Podívej se na slůvko „jsi“ za slovesem — s kterým zájmenem se pojí?" },
  { i: 5, par: ["běhal jsem", "běhal jsi", "běhal", "běhali jsme", "běhali jste", "běhali"], tip: "Za slovesem žádné „jsem, jsi, jsme, jste“ není. Co to znamená pro osobu?" },
  ...
  { i: 4, par: ["dělal jsem", "dělal jsi", "dělal", "dělali jsme", "dělali jste", "dělali"], tip: "Rozhoduje slůvko „jste“ — liší se od „jsme“ jediným písmenem." },
  { i: 0, par: ["nesl jsem", "nesl jsi", "nesl", "nesli jsme", "nesli jste", "nesli"], tip: "Slůvko „jsem“ za slovesem napoví osobu; tvar „nesl“ napoví, kolik lidí nese." },
...
function ulohaL2({ i, par, zajmena = ZAJMENA, tip }: L2Item): PracticeTask {
  const tvar = par[i];
  ...
  const distr = [opacneCislo, ...stejneCislo].map((k): Distractor => ({
    value: osCis(k),
    why: `${osCis(k)} by bylo „${par[k]}“ (${zajmena[k]} ${par[k]}), ne „${tvar}“.`,
  })) as [Distractor, Distractor, Distractor];
  return choice(`Urči osobu a číslo slovesa „${tvar}“.`, osCis(i), distr, {
    hints: [
      `Které zájmeno se hodí před „${tvar}“: já, ty, on, my, vy, nebo oni?`,
      `${tip} Řekni si „${tvar}“ se zájmeny a vyber spojení, které zní správně. Zájmeno prozradí osobu (já/my první, ty/vy druhá, on/oni třetí) i číslo (jeden, nebo víc lidí).`,
    ],
    explanation: `Říkáme „${zajmena[i]} ${tvar}“. Zájmeno „${zajmena[i]}“ patří k ${(i % 3) + 1}. osobě a ${i < 3 ? "mluví o jednom, proto je číslo jednotné" : "mluví o více lidech, proto je číslo množné"}.`,
  });
}
