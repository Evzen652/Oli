/**
 * Sdílené utility pro přírodopis 6. ročníku (faktický předmět).
 *
 * Stavba úloh je stejná jako v dějepise (`../dejepis/_shared.ts`), liší se
 * rady ve velké nápovědě — dějepisné („kámen byl dřív než kov“) by tu mátly —
 * a `buildChoiceTask` vrací `null`, když po vyřazení shod nezbydou tři různé
 * distraktory (stejná pojistka jako v matematice). Generátor pak úlohu
 * vylosuje znovu přes `losUlohy()`.
 *
 * Distraktor = typická přírodopisná miskoncepce (houba je rostlina, pavouk je
 * hmyz, antibiotika léčí chřipku, velryba je ryba…), ne náhodný pojem.
 */
...
const STRATEGIE_VYBER = [
  "Vylučuj možnosti, které odporují tomu, co o organismu víš.",
...
const STRATEGIE_TRIDENI = [
  ...
  "U každé položky se ptej, podle jakého znaku do skupiny patří, ne jak vypadá na první pohled.",
];
