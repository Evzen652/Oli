...
//
// Opravený klíč: u řady se střídavými kroky (+p, −m) se vypisuje šest členů,
// takže poslední použitý krok je +p a na řadě je −m. Předchozí verze vracela
// jako klíč xs[5] + p a správnou odpověď xs[5] − m nabízela jako distraktor.
...
function hintBezCisla(key: number, hints: [string, string]): boolean {
  const s = String(key);
  return !hints.some((h) => (h.match(/\d+/g) ?? ([] as string[])).includes(s));
}
...
/** Tvar po číslovce ve větě „má N …" (4. pád). `Co` nese 2. pád z otázky „Kolik …". */
const POCITANE: Record<Co, [string, string, string]> = {
  "hran": ["hranu", "hrany", "hran"],
  "stěn": ["stěnu", "stěny", "stěn"],
  "vrcholů": ["vrchol", "vrcholy", "vrcholů"],
};
...
function teleso(): PracticeTask | null {
  ...
  return ciselnaUloha(`Kolik ${co} má ${t.nazev}?`, key, [
    ...
    `Proto má ${t.nazev} ${key} ${plural(key, ...POCITANE[co])} — počítá se po skupinách, které se nepřekrývají.`,
  ]);
}
...
function stridava(): PracticeTask | null {
  ...
  return ciselnaUloha(q, key, [
    ...
    { value: xs[5] + (p - m), why: `Za celou dvojici kroků (+${p} a −${m}) řada vyroste o ${p - m}. Teď je ale na řadě jen jeden krok, a to ten dolů.` },
...
function obleceni(): PracticeTask | null {
  ...
  const hints: [string, string] = [
    ...
    `K prvnímu tričku se hodí kterákoli sukně, takže s ním samotným máš ${pad(k, "MOŽNOST")}. Úplně stejně to dopadne u druhého trička i u každého dalšího — a Petra jich má ${t}, takže se stejný počet možností opakuje ${t}krát.`,
  ];
  return ciselnaUloha(`Petra má ${tricka} a ${sukne}. Kolik různých dvojic tričko a sukně si může vybrat?`, t * k, [
    ...
    `Petra má ${pad(t, "TRIČKO")} a u každého se ta nabídka opakuje: ${t} × ${k} = ${t * k}`,
  ]);
}
...
function kvadr(): PracticeTask | null {
  ...
  const hints: [string, string] = [
    `Rozděl si kvádr na vodorovné vrstvy. Spodní vrstva je obdélník ${a} na délku a ${b} na šířku a takových vrstev leží na sobě ${c}. Kolik kostek je v té jedné vrstvě?`,
    `Nejdřív spočítej kostky v jedné vrstvě — je jich tolik, kolik dá ${a} × ${b}. Takových stejných vrstev leží na sobě ${c}, takže počet z jedné vrstvy vezmi ještě ${c}krát; sečítat rozměry by nedávalo smysl, protože kostky vyplňují prostor.`,
  ];
  return ciselnaUloha(`Kvádr je složený z kostek: na délku ${a}, na šířku ${b} a na výšku ${c}. Kolik kostek má celý kvádr?`, a * b * c, [
    ...
    `Jedna vrstva: ${a} × ${b} = ${pad(a * b, "KOSTKA")}`,
...
function kvadrInverze(): PracticeTask | null {
  ...
  const hints: [string, string] = [
    ...
    `V jedné vrstvě je tolik kostek, kolik dá ${a} × ${b} — spočítej to. Celý kvádr je z takových stejných vrstev poskládaný, takže zjisti, kolikrát se počet kostek jedné vrstvy vejde do ${celkem}; to se dělá dělením, ne odčítáním.`,
  ];
  ...
  return ciselnaUloha(q, c, [
    ...
    `Jedna vrstva: ${a} × ${b} = ${pad(vrstva, "KOSTKA")}`,
...
function vek(): PracticeTask | null {
  ...
  const hints: [string, string] = [
    `${A} a ${B} mají dohromady ${pad(S, "ROK")}. Nakresli si jejich věky jako dva sloupečky — ten delší má o ${pad(d, "ROK")} víc. Co s ním uděláš, aby byly oba stejné?`,
    `${A} a ${B} mají dohromady ${pad(S, "ROK")} a liší se o ${pad(d, "ROK")}. Odečti ten rozdíl od součtu — co zbude, patří dvěma stejně starým dětem, takže se to dá rozdělit na dvě stejné poloviny. Tak zjistíš věk mladšího z nich a rozdíl potom k němu přičti zpátky, protože otázka se ptá na toho staršího.`,
  ];
  if (!bezLeaku(q, key, hints) || !hintBezCisla(key, hints)) return null;
  return ciselnaUloha(q, key, [
    ...
    `Starší je o ${pad(d, "ROK")} víc: ${m} + ${d} = ${key}. Zkouška: ${m} + ${key} = ${S} ✓`,
  ]);
}
