...
//
// Doplněno 2026-09-12 (inventura obsahu): nápovědy byly parametrizované jen
// dělitelem, takže se jedna věta opakovala až sedmkrát, a velká nápověda
// nebyla delší než malá. Teď obě nesou dělence i dělitele té úlohy a velká
// rozepisuje celý postup včetně zkoušky.
...
function bezZbytku(dMin: number, dMax: number, qMin: number, qMax: number, nMin: number, nMax: number): PracticeTask | null {
  ...
  return ciselnaUloha(`Vypočítej písemně: ${fmt(N)} : ${d}`, q, [
    ...
    { value: q + 10, why: `Odhad první číslice podílu byl o jedna vyšší. Zkouška: ${q + 10} × ${d} = ${fmt((q + 10) * d)}, a to je hodně přes ${fmt(N)}.` },
