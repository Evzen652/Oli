...
function sit(): PracticeTask | null {
  ...
  return overeno(ciselnaUloha(
    ...
    [
      `Jeden řádek pokrývá ${pad(s, "ČTVEREČEK")} a řádků je celkem ${r}. Sčítat je jeden po druhém by trvalo dlouho — která operace opakované sčítání zkrátí?`,
...
function obdelnik(): PracticeTask | null {
  ...
  return overeno(ciselnaUloha(
    ...
    S(a * b),
    [
      { value: S(2 * (a + b)), why: `${fmt(2 * (a + b))} je obvod — délka čáry kolem obdélníku. Obsah je plocha uvnitř a počítá se násobením stran.` },
      { value: S(a + b), why: `Strany se sečetly. Součet ${a} + ${b} říká, jak jsou dlouhé dohromady, ne kolik čtverečků obdélník pokryje.` },
      { value: `${fmt(a * b)} ${u}`, why: `Číslo je spočítané správně, ale jednotka ne: počítají se čtverečky 1 ${u} × 1 ${u}, takže obsah patří do ${u}², ne do ${u}.` },
...
function ctverec(): PracticeTask | null {
  ...
  return overeno(ciselnaUloha(
    ...
    [
      ...
      { value: S(2 * a), why: `Strana se vynásobila dvěma. Čtverec se stranou ${a} ${u} má ale ${a} ${plural(a, "řadu", "řady", "řad")} po ${a} čtverečcích, ne dvě řady.` },
    ...
    [
      `Čtverec se stranou ${a} ${u} má ${a} ${plural(a, "řadu", "řady", "řad")} po ${a} čtverečcích.`,
      `${a} × ${a} = ${a * a}`,
      `Obsah je ${S(a * a)}.`,
    ],
  ));
...
function prirustek(): PracticeTask | null {
  ...
  return overeno(ciselnaUloha(
    ...
    [
      ...
      { value: S(a * c), why: `Prodloužení ${c} cm se vynásobilo prodlužovanou stranou ${a} cm. Nový pruh je ale dlouhý tolik, kolik měří strana, která se nezměnila.` },
      { value: S(c), why: `${c} je jen délka, o kterou se strana prodloužila. Přírůstek obsahu je celý nový pruh, ne jen jeho šířka.` },
    ],
    [
      ...
      `Přírůstek je sám obdélník: jedna jeho strana měří ${c} cm (to je to prodloužení), druhá je ta strana, která se nezměnila. Jeho obsah spočítáš jako délka × šířka. Nemusíš tedy počítat obsah nového obdélníku a odečítat od něj starý — obojí vyjde stejně, ale přes přidaný pruh je to na jeden krok.`,
    ],
    [
      `Nový pruh je obdélník: jedna strana měří ${c} cm, druhá ${b} cm.`,
      `${b} × ${c} = ${b * c}`,
...
const PREVODY: Prevod[] = [
  ...
  },
  {
    ...
    pravidlo: "Čtverec se stranou 1 km má stranu dlouhou 1 000 m a vedle sebe se na ni vejde deset hektarových čtverců se stranou 100 m, takže se do celého čtverce vejde deset řad po deseti hektarech.",
  },
];
...
function prevodNaVetsi(): PracticeTask | null {
  ...
  return overeno(ciselnaUloha(
    ...
    [
      ...
      // Pozor: `k * p.delkove` vyjde u převodů s f = 100 stejně jako `k * 10`,
      // takže dřívější třetí distraktor s nimi splýval a ciselnaUloha kvůli
      // tomu vracela null — tři z pěti převodů se na L3 vůbec neobjevovaly.
      // Přehnané dělení (o řád navíc) dá vždy jinou hodnotu než obě předchozí.
      { value: `${fdec(k / 10)} ${p.velka}`, why: `Dělilo se číslem o řád větším: do ${p.velka2} se vejde ${fmt(p.f)} ${p.mala}, ne ${fmt(p.f * 10)}.` },
    ],
    [
...
function dvaKroky(): PracticeTask | null {
  ...
  return overeno(ciselnaUloha(
    ...
    [
      ...
      `Obsah obdélníku ${a} dm × ${b} dm vyjde ve čtverečních decimetrech, protože jsou v nich zadané obě strany. Čtvereční decimetr je čtverec 1 dm × 1 dm a jeho strana měří 10 cm — vejde se do něj deset řad po deseti čtverečcích 1 cm × 1 cm. Druhý krok je proto násobení stem, ne deseti.`,
    ],
    [
...
function stranaCtverce(): PracticeTask | null {
  ...
  return overeno(ciselnaUloha(
    ...
    [
      ...
      { value: `${2 * s} cm`, why: `Zkouška: ${2 * s} × ${2 * s} = ${4 * s * s}, ne ${s * s}.` },
...
function druhaStrana(): PracticeTask | null {
  ...
  return overeno(ciselnaUloha(
    ...
    [
      ...
      { value: `${b + 2} cm`, why: `Zkouška: ${a} × ${b + 2} = ${a * (b + 2)}, ne ${S}.` },
    ],
    [
