...
function naOse(): PracticeTask | null {
  ...
  return overeno(ciselnaUloha(
    ...
    [
      ...
      { value: Z(-(n - 1)), why: `O jeden díl blíž nule. Počítej znovu — po prvním dílu od nuly doleva stojíš na čísle ${Z(-1)}.` },
    ],
    [
...
function teplomer(): PracticeTask | null {
  ...
  return overeno(ciselnaUloha(
    ...
    [
      ...
      { value: "0 °C", why: "Nula je právě bod mrazu. Teplota pod nulou leží na ose vlevo od něj." },
...
function hloubka(): PracticeTask | null {
  ...
  return overeno(ciselnaUloha(
    `${kdo} je ${n} m pod hladinou. Jak tu polohu zapíšeš číslem, když je hladina 0 m?`,
    `${Z(-n)} m`,
    [
      { value: `${n} m`, why: `${n} m bez znaménka by znamenalo ${n} m nad hladinou, třeba na stožáru.` },
      { value: "0 m", why: "Nulu má samotná hladina. Místo pod hladinou má číslo menší než nula." },
...
function mezi(): PracticeTask | null {
  ...
  // Distraktor `Z(-a)` je kladný a jeho vysvětlení stojí na tom, že úsek leží
...
function opacne(): PracticeTask | null {
  ...
  return overeno(ciselnaUloha(
    ...
    Z(-n),
    [
      { value: "0", why: `Nula leží přesně uprostřed mezi hledanou dvojicí, takže od nuly není vzdálená vůbec — a od čísla ${n} je vzdálená ${pad(n, "DÍL")}.` },
...
function zpetnaTeplota(): PracticeTask | null {
  ...
  return overeno(ciselnaUloha(
    ...
    `${Z(rano)} °C`,
    [
      { value: `${Z(konec + zmena)} °C`, why: `Oteplení se přičetlo, místo aby se odečetlo. Ráno bylo chladněji než v poledne, takže se od ${Z(konec)} °C jde po ose zpátky doleva.` },
      { value: `${Z(-rano)} °C`, why: `Číslo sedí, znaménko ne: posun o ${pad(zmena, "DÍL")} doleva z ${Z(konec)} končí vlevo od nuly.` },
      {
        value: "0 °C",
        why: konec > 0
          ? `U nuly cesta zpátky neskončila: z ${konec} °C k nule stačí ${pad(konec, "DÍL")}, ale odečíst se má ${zmena} °C.`
          : `Nula leží vpravo od ${Z(konec)} °C, ale cesta zpátky vede opačným směrem — doleva, ještě dál od nuly.`,
      },
    ],
    [
