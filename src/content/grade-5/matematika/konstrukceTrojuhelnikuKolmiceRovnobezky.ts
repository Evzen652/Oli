...
//
// Doplněno 2026-09-12 (inventura obsahu): L1 měla jen 13 úloh, těsně nad
// hranicí — přibylo pět (zápis kolmosti a rovnoběžnosti, ostrý a tupý úhel,
// kružítko). Velké nápovědy u výpočtových úrovní byly společné pro celou
// skupinu, teď nesou čísla té úlohy. Zadání 60°, 60° se zahazuje: takový
// trojúhelník je i rovnostranný, takže by měl dvě obhajitelné odpovědi.
// Čtverec a obdélník vypadly z L3 — jejich vlastnosti jsou látka L1.
...
function tretiUhel(): PracticeTask | null {
  ...
  return ciselnaUloha(`V trojúhelníku má jeden úhel ${a}° a druhý ${b}°. Kolik stupňů má třetí úhel?`, c, [
    ...
    { value: 360 - a - b, why: `Tohle číslo vznikne, když se oba úhly odečtou od 360°, což je součet úhlů čtyřúhelníku. Trojúhelník má součet 180°.` },
...
function nerovnost(): PracticeTask | null {
  ...
  return ciselnaUloha(`Lze sestrojit trojúhelník se stranami ${poradi[0]} cm, ${poradi[1]} cm a ${poradi[2]} cm?`, lze ? ANO : NE, [
    { value: lze ? NE : ANO, why: `${s1} + ${s2} = ${s1 + s2}, a to je ${lze ? `víc než ${s3}` : s1 + s2 === s3 ? `přesně tolik jako ${s3}` : `méně než ${s3}`}.${!lze && s1 + s2 === s3 ? " Při rovnosti by se strany složily do úsečky." : ""}` },
...
// „obecný lichoběžník", ne pouhý „lichoběžník": bez přívlastku by otázka na
// pravé úhly neměla jedinou odpověď — pravoúhlý lichoběžník je taky lichoběžník
// a stojí v téhle tabulce hned vedle.
const TVARY: [string, number, number][] = [
  ["kosočtverec (který není čtverec)", 2, 0], ["obecný lichoběžník", 1, 0], ["pravoúhlý lichoběžník", 1, 2], ["obecný čtyřúhelník", 0, 0], ["kosodélník", 2, 0],
];
...
/** Krátký tvar do věty „má dvě dvojice…, ne jednu" — ať se souslovi neopakuje. */
const R_KRATCE: Record<number, string> = { 0: "žádnou", 1: "jednu", 2: "dvě", 4: "čtyři" };
  ...
  const chyby = [0, 1, 2, 4].filter((m) => m !== key).map((m) => ({
    ...
    why: naRov
      ? (m === 4
        ...
        // U rov === 0 je `stav` sám záporný („nemá žádné dvě strany rovnoběžné"),
        // takže připojené „, ne jednu" by se s tím záporem tlouklo.
        : rov === 0
          ? `${stav}, takže dvojic je nula. Porovnávej vždy dvojici stran, které leží proti sobě.`
          : `${stav}, ne ${R_KRATCE[m]}. Porovnávej vždy dvojici stran, které leží proti sobě.`)
