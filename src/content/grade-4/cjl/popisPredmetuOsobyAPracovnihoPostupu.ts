...
const POOL_L1: QA[] = [
  { q: "Co popisujeme v popisu předmětu?", a: "tvar, barva, materiál, účel", opts: ["příběh, jak vznikl", "tvar, barva, materiál, účel", "jeho historii a stáří", "pocity a dojmy z něj"], e: "Popis předmětu zachycuje jeho vlastnosti — jak vypadá (tvar, barva), z čeho je (materiál), jak velký je a k čemu slouží. Pocity, příběh ani historie sem nepatří, ty bychom psali do vypravování nebo úvahy." },
  ...
  { q: "Co popisujeme v popisu osoby?", a: "vzhled i povahu", opts: ["jen jeho oblečení", "vzhled i povahu", "jen to, co dělá", "jen věk a jméno"], e: "Dobrý popis osoby spojuje to, jak člověk vypadá (vzhled), s tím, jaký je (povaha). Jen oblečení nebo jen věk a jméno by k představě člověka nestačily." },
  { q: "Popis pracovního postupu musí být:", a: "seřazený po krocích", opts: ["poetický a obrazný", "seřazený po krocích", "volný, bez pořadí", "pozpátku od konce"], e: "Pracovní postup popisuje, jak něco udělat krok za krokem, proto musí jít v pořadí od prvního kroku k poslednímu. Kdyby kroky byly přeházené, postup by nešel provést." },
...
const POOL_L2: QA[] = [
  ...
  { q: "Jaký je správný popis osoby (fyzický)?", a: "Jan je vysoký a tmavovlasý", opts: ["Jan říkal: 'Jsem vysoký.'", "Jan je vysoký a tmavovlasý", "Jan je hodný a má rád fotbal", "Jan žil v malém městě"], e: "Fyzický popis uvádí znaky vzhledu — výšku, vlasy, oči, postavu. 'Je hodný a má rád fotbal' popisuje povahu a zájmy, ne vzhled, a 'žil v malém městě' je údaj z vypravování." },
  { q: "Co chybí v popisu pracovního postupu: '1. Vezmi mouku. 2. Přidej vejce. 3. ... 4. Upečeme koláč.'?", a: "Chybí krok číslo 3", opts: ["Nic tam nechybí", "Chybí krok číslo 3", "Chybí jméno autora", "Chybí datum pečení"], e: "Mezi 'přidej vejce' a 'upečeme koláč' chybí krok, ve kterém se těsto dodělá a smíchá — bez něj by postup nešel dokončit. Datum ani jméno autora do pracovního postupu nepatří." },
  { q: "Kterými slovy určíme polohu detailu?", a: "nahoře, dole, uprostřed", opts: ["pak, potom, nakonec", "nahoře, dole, uprostřed", "prvně, za druhé, za třetí", "hodný, zlý, rychlý"], e: "Polohu částí předmětu vyjadřují slova o místě — nahoře, dole, vlevo, uprostřed. Slova pak, potom označují čas (pracovní postup) a hodný, zlý popisují povahu." },
  ...
  { q: "Jaký popis je přesnější?", a: "Koláč byl kulatý, zlatohnědý", opts: ["Koláč jsem nakonec snědl", "Koláč byl kulatý, zlatohnědý", "Koláč byl pěkný a dobrý", "Koláč byl z naší kuchyně"], e: "Přesnější popis uvádí konkrétní vlastnosti — tvar, barvu, rozměr. 'Pěkný a dobrý' je jen obecné hodnocení a 'snědl jsem ho' je děj, ne popis." },
  ...
  { q: "V jakém slohovém útvaru popíšeme, jak upéct chleba?", a: "popis pracovního postupu", opts: ["vypravování o pečení", "popis pracovního postupu", "popis předmětu chleba", "popis osoby pekaře"], e: "Jak něco udělat krok za krokem popisuje pracovní postup, jehož typickým příkladem je recept. Popis předmětu by řekl, jak chléb vypadá, ne jak ho upéct." },
  { q: "Příklad nepřesného popisu předmětu — jaká chyba je v textu: 'Stůl je velký a dobrý.'?", a: "chybí rozměry a materiál", opts: ["chybí jen barva stolu", "chybí rozměry a materiál", "text je úplně správný", "chybí jen hmotnost"], e: "Slova 'velký' a 'dobrý' jsou příliš obecná a 'dobrý' navíc nic nepopisuje — chybí konkrétní rozměr a materiál (např. 'dřevěný stůl 120 cm dlouhý'). Nejde tedy jen o barvu nebo hmotnost." },
...
function gen(level: number): PracticeTask[] {
  ...
  return selected.map(({ q, a, opts, e }) => ({
    ...
    hints: [
      "Nejdřív si rozmysli, o čem vlastně píšeš: o věci, o člověku, nebo o činnosti.",
      "U věci i u člověka začni tím největším a nejnápadnějším — tím, čeho si všimneš na první pohled — a teprve pak přidávej drobnosti, které uvidíš až zblízka.",
      "U činnosti je to jinak: tam se řídíš časem a držíš přesně to pořadí, v jakém jdou jednotlivé části po sobě, od první až k poslední.",
    ],
    explanation: e,
  }));
}
