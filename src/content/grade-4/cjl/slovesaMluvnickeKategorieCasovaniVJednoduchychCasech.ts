...
const POOL_L2: QA[] = [
  ...
  { q: "Urči osobu a číslo: 'prší'", a: "3. osoba jednotného čísla (neosobní)", opts: ["2. osoba jednotného čísla", "3. osoba jednotného čísla (neosobní)", "1. osoba jednotného čísla", "nelze to vůbec určit"], e: "U sloves jako 'prší' nikdo děj nevykonává (neříkáme 'já prším'), proto je řadíme k 3. osobě jednotného čísla a nazýváme je neosobní. Osobu sice přímo nevidíme, ale určit ji lze." },
  ...
  { q: "Co je neurčitek (infinitiv)?", a: "základní tvar slovesa — 'číst, psát, jít'", opts: ["přítomný čas, tedy tvar s osobou", "základní tvar slovesa — 'číst, psát, jít'", "minulý čas, tedy tvar s příčestím", "rozkazovací způsob, tedy příkaz"], e: "Neurčitek je základní tvar slovesa, který sám neurčuje osobu ani čas — končí nejčastěji na -t (číst, psát, jít). Teprve když ho začneme časovat ('čtu', 'četl jsem'), získá osobu a čas." },
  { q: "Urči osobu a číslo: 'doběhla'", a: "3. osoba jednotného čísla (ona)", opts: ["3. osoba množného čísla", "3. osoba jednotného čísla (ona)", "1. osoba jednotného čísla", "2. osoba jednotného čísla"], e: "Tvar 'doběhla' patří k zájmenu 'ona' — to je 3. osoba a jeden člověk, tedy jednotné číslo. Kdyby doběhlo víc dívek, řekli bychom 'doběhly' (množné číslo)." },
...
const POOL_L3: QA[] = [
  ...
  { q: "Jaký je rozdíl mezi 'číst' a 'čtu'?", a: "číst = infinitiv (neurčitek), čtu = přítomný čas, 1. os. j.č.", opts: ["obojí je stejný neurčitý tvar slovesa", "číst = infinitiv (neurčitek), čtu = přítomný čas, 1. os. j.č.", "obojí vyjadřuje stejný přítomný čas", "číst je naopak tvar v minulém čase"], e: "'Číst' je neurčitek — základní tvar bez osoby a času, zatímco 'čtu' je už vyčasované sloveso (já, teď, přítomný čas). Proto nemohou být obě stejná ani obě infinitivem." },
  ...
  { q: "Urči způsob: 'kdybychom přišli'", a: "podmiňovací (podmínková věta)", opts: ["rozkazovací, jako příkaz", "podmiňovací (podmínková věta)", "přací, jako vyjádření přání", "oznamovací, jako obyčejné sdělení"], e: "Slovo 'kdybychom' v sobě skrývá 'bychom' a uvozuje podmínku — proto jde o podmiňovací způsob. Vyjadřuje něco, co by se stalo, jen kdyby platila daná podmínka." },
  ...
  { q: "Urči způsob: 'at' přijdou'", a: "přací / rozkazovací (ať + sloveso)", opts: ["podmiňovací, jako by tam bylo 'by'", "přací / rozkazovací (ať + sloveso)", "neurčitý, protože tam chybí osoba", "oznamovací, jako obyčejné sdělení"], e: "Spojení 'ať' + sloveso vyjadřuje přání nebo výzvu — řadíme ho k přacímu (rozkazovacímu) způsobu. Oznamovací by jen sděloval, podmiňovací by obsahoval 'by'." },
  { q: "Urči čas: 'bývalo'", a: "minulý (opakovaný děj v minulosti)", opts: ["přítomný, tedy děj probíhající teď", "minulý (opakovaný děj v minulosti)", "podmiňovací, jako by tam bylo 'by'", "budoucí, tedy děj, který teprve nastane"], e: "Tvar 'bývalo' popisuje děj, který se v minulosti opakoval — pořád je to čas minulý. Přítomnost by byla 'bývá', budoucnost 'bude bývat'." },
...
function gen(level: number): PracticeTask[] {
  ...
  return selected.map(({ q, a, opts, e }) => ({
    ...
    hints: [
      "Osoba ukazuje, kdo přesně sloveso vykonává — mluvčí, posluchač, nebo někdo třetí.",
      "Číslo ukazuje, jestli děj koná jeden člověk, nebo víc lidí najednou.",
      "Sloveso může popisovat děj, který už proběhl, který právě probíhá, nebo který teprve nastane.",
      "Sloveso může něco oznámit, přikázat, nebo vyjádřit podmínku, za které by se něco stalo.",
    ],
    explanation: e,
  }));
}
