...
const POOL_L1: PoolItem[] = [
  ...
  { question: "Jak pohádky obvykle začínají?", correct: "Frází jako 'Byl jednou jeden...' nebo 'Za devatero horami...'", distractors: ["Větou 'Včera odpoledne se u nás stalo...'", "Otázkou 'Co je to za zvíře?'"], emoji: "📖", hint: "Pohádkový začátek nás uvádí do dávných dob nebo vzdálených zemí — jak zní?", solution: "Pohádky začínají frázemi jako 'Byl jednou jeden...' nebo 'Za devatero horami...' — uvádí nás do pohádkové doby." },
  ...
  { question: "Co je typické pro říkanku?", correct: "Konce řádků se rýmují", distractors: ["Vždy popisuje zvíře", "Vždy klade otázku"], emoji: "🎵", hint: "Říkanka: 'Kolo, kolo mlýnské...' — všimni si, jak poslední slova v každém řádku znějí podobně.", solution: "V říkankách se konce řádků rýmují — to je jejich typický znak. Díky tomu se hezky říkají nahlas." },
...
const POOL_L3: PoolItem[] = [
  { question: "Čím se říkanka liší od básně?", correct: "Říkanka slouží ke hře, báseň vyjadřuje pocity", distractors: ["Říkanka je vždycky delší než báseň", "Říkanka se nerýmuje, ale báseň ano"], emoji: "🎵", hint: "Obě se rýmují — v čem tedy spočívá jejich rozdíl?", solution: "Říkanka slouží ke hře a opakování, báseň vyjadřuje hlubší pocity — obě se rýmují, ale každá slouží k něčemu jinému." },
  { question: "Kdo píše básně?", correct: "Básník nebo básnířka", distractors: ["Fyzik nebo fyzička", "Kuchař nebo kuchařka"], emoji: "✍️", hint: "Básně jsou umělecká díla — kdo je tvoří a jak mu říkáme?", solution: "Básně píše básník nebo básnířka — lidé, kteří umělecky tvoří rýmované texty." },
