...
const before = raw.split(" q: ").length - 1;
let ok = 0, fail = 0;
for (const [oldKey, key, dis] of T) {
  const needle = ' a: "' + oldKey;
  const hits = [];
  ...
  const ob = line.indexOf("opts: [", ca);
  const oe = ob === -1 ? -1 : line.indexOf("]", ob);
  if (oe === -1) { console.log("!! opts: " + oldKey.slice(0, 40)); fail++; continue; }
  const opts = [dis[0], key, dis[1], dis[2]].map(o => '"' + o + '"').join(", ");
  console.log("  radek " + (li + 1) + " <- " + oldKey.slice(0, 32));
  if (APPLY) L[li] = line.slice(0, ca) + ' a: "' + key + '", opts: [' + opts + line.slice(oe);
  ok++;
}
...
if (APPLY) {
  const out = L.join(nl);
  const after = out.split(" q: ").length - 1;
